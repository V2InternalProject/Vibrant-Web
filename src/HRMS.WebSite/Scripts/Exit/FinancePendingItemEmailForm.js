function DisplayLoadingDialog() {
 ...
 function FinanceSendMail() {
	...
	if (counter == 1) {
		...
		var Parameter = { employeeId: EmpIdToPassFinance }
		$.ajax({
			url: MailUrl,
			type: 'GET',
			cache: false,
			data: { "employeeId": EmpIdToPassFinance, "collection": collection },
						...
						if ($('#MailDetails').valid()) {
							...
							$.ajax({
								...
								success: function (data) {
									...
									//$('#btnFinanceSendPendingItem').dialog('destroy');
