...
app.controller('MyCtrl', ["$scope", "AppraislFactory","$http", function ($scope, AppraislFactory,$http) {
    ...
    //AppraislFactory.getGridData().AddNewPropertyToCollection([{"prop":"checker","value":"false"}]);
    AppraislFactory.getGridData().success(function (data, status, headers) {
        $scope.myData = data;
        //$scope.gridInitiatedOptions.$gridScope.columns[11].toggleVisible();
        //$scope.gridInitiatedOptions.$gridScope.columns[12].toggleVisible();
    ...
    $scope.OK = function (newValue) {
        if (saveEmployeeList.length > 0) {
            $scope.myDiv = true;
            AppraislFactory.cancelGridInitiation(newValue, saveEmployeeList)
                 .success(function (data, status, headers) {
                     if (data == '1') {
                         alert("Data is Freezed");
                         window.location.reload();
                     }
                 }).error(function (data) {
                     $scope.myDiv = false;
    ...
    $scope.selectAll = function (rowCheck) {
        angular.forEach($scope.gridInitiatedOptions.$gridScope.renderedRows, function (elem) {
            if (rowCheck == false) {
...
.filter('mapStatus', function (AppraislFactory) {
 ...
 .factory('AppraislFactory', ['$http', function ($http) {
     var gridData = function () {
         return $http({
             method: 'GET',
             url: '/api/AppraisalReview/Initiated',
         })
     }
     ...
     var cancelRequest = function (newValue, cancelList) {
         var jsonData = { lstAppr: JSON.stringify(cancelList), message: newValue };
       return $http({
           method: 'POST',
           url: '/api/AppraisalReview/Freeze',
           //params: ,
           data: { lstAppr: cancelList, message: newValue },
          dataType: "json",
           //headers: {
    ...
    //     $.post('/api/AppraisalReview/Freeze', { lstAppr: cancelList},
    ...
    //});
     ...
     return {
         ...
         cancelGridInitiation: function (newValue, cancelList) { return cancelRequest(newValue, cancelList); }
     }
 }])