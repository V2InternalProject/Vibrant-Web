...
app.controller('MyCtrl', ["$scope", "AppraislFactory", "$http",function ($scope, AppraislFactory,$http) {
    ...
    $scope.gridReviewOptions = {
        data: 'myData',
        enableRowSelection: false,
        enableCellEditOnFocus: false,
        //checkboxTemplate:'<input type="checkbox" ng-model="COL_FIELD" ng-change="checkedIndex(row)"/>',
    ...
    //    cellTemplate: '<input type="checkbox" ng-model="COL_FIELD" ng-change="checkedIndex(row)">'
    //},
...
{ field: "DUName", width: "*", displayName: 'Delivery Unit', enableCellEdit: false }]
    ...
    AppraislFactory.getGridData().success(function (data, status, headers) {
        //data.AddNewPropertyToCollection([{ "prop": "checker", "value": "false" }]);
        $scope.myData = data;
        //$scope.gridReviewOptions.$gridScope.columns[11].toggleVisible();
        //$scope.gridReviewOptions.$gridScope.columns[12].toggleVisible();
    }).error(function (data) {
        $scope.message = data.Message;
    ...
    $scope.cancelReviewGrid = function () {
        if (saveEmployeeList.length > 0) {
            $scope.myDiv = true;
    ...
    $scope.selectAll = function (rowCheck) {
        angular.forEach($scope.gridReviewOptions.$gridScope.renderedRows, function (elem) {
            if (rowCheck == false) {
...
.filter('mapStatus', function (AppraislFactory) {
 ...
 .factory('AppraislFactory', ['$http', function ($http) {
     var gridData = function () {
         return $http({
             method: 'GET',
             url: '/api/AppraisalReview/ReadytoInitiate',
         })
     }
     ...
     var saveRequest = function (saveList) {
         ...
         return $http({
             method: 'POST',
             url: '/api/AppraisalReview/Initiate',
             data: saveList,
             dataType: "json"
         });
     }
