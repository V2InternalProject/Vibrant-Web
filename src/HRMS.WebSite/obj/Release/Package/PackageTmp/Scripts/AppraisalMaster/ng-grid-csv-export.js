...
function ngGridCsvExportPlugin(opts, $http) {
    var self = this;
    self.grid = null;
    self.scope = null;
    self.services = null;
    self.init = function (scope, grid, services) {
        ...
        console.log(scope.$parent.$parent.$parent.statuses);
        var doRequestTest = [];
        $http.get('/api/AppraisalReview/GetSetupList').
            ...
            for (var gridRow in gridData) {
                for (k in keys) {
                    ...
                    if (opts != null && opts.columnOverrides != null && opts.columnOverrides[keys[k]] != null) {
                        ...
                        if (keys[k] == "App1" || keys[k] == "App2" || keys[k] == "Rv1" || keys[k] == "RV2" || keys[k] == "GHID" || keys[k] == "IDF" || keys[k] == "IDFE1" || keys[k] == "IDFE2") {
                            curCellRaw = FilterDropDown(curCellRaw, doRequestTest);
                        }
                    }
...
function FilterDropDown(columnValue, doRequest) {
    if (columnValue != 0) {
        var emplyeeMatched = doRequest.Appr1.filter(function (emp) {
            if (columnValue.Value == undefined) {
                return emp.Value == columnValue;
            }
            else {
                return emp.Value == columnValue.Value;
            }
        });
